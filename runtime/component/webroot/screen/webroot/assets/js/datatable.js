    ...
    return {
        ...
        init: function (options) {
            ...
            options = $.extend(true, {
                ...
                dataTable: {
                    ...
                    "fnServerData": function ( sSource, aoData, fnCallback, oSettings ) {
                      oSettings.jqXHR = $.ajax( {
                        ...
                        "success": function(res, textStatus, jqXHR) {
                            if (res.sMessage) {
                                CoreApp.alert({type: (res.sStatus == 'OK' ? 'success' : 'danger'), icon: (res.sStatus == 'OK' ? 'check' : 'warning'), message: res.sMessage, container: tableWrapper, place: 'prepend'});
                            }
                            if (res.sStatus) {
                        ...
                        "error": function() {
                            ...
                            CoreApp.alert({type: 'danger', icon: 'warning', message: tableOptions.dataTable.oLanguage.sAjaxRequestGeneralError, container: tableWrapper, place: 'prepend'});
                    ...
                    "fnDrawCallback": function( oSettings ) { // run some code on table redraw
                        ...
                        CoreApp.initUniform($('input[type="checkbox"]', table));  // reinitialize uniform checkboxes on each table reload
            ...
            table.on('click', '.filter-submit', function(e){
                ...
                $('.filter textarea.form-control, .filter select.form-control, .filter input.form-control:not([type="radio"],[type="checkbox"])', table).each(function () {
                    the.addAjaxParam($(this).attr("name"), $(this).val());
                });
                ...
                $('.filter input.form-control[type="checkbox"]:checked, .filter input.form-control[type="radio"]:checked', table).each(function () {
                    the.addAjaxParam($(this).attr("name"), $(this).val());
                });
            ...
            table.on('click', '.filter-cancel', function(e){
                e.preventDefault();
                $('.filter textarea.form-control, .filter select.form-control, .filter input.form-control', table).each(function(){
                    $(this).val("");
                });
                $('.filter input.form-control[type="checkbox"]', table).each(function(){
                    $(this).attr("checked", false);
                });
                $('.filter select.form-control.select2me', table).each(function(){
                    $(this).select2("val", "");
                })
